...
document.addEventListener('DOMContentLoaded', () => {
  const LifestyleModule = {
    ...
    calculateAndRenderStats() {
      ...
      document.getElementById('travel-stat-trips-count').textContent = activeTripsCount;
      document.getElementById('travel-stat-upcoming-badge').textContent = `${upcomingCount} Upcoming`;
      ...
      document.getElementById('travel-stat-days-count').textContent = totalDays;
      document.getElementById('travel-stat-days-badge').textContent = `+${Math.max(0, totalDays - 12)} days vs last year`;

      // 3. Places Visited
      const countries = new Set(places.map(p => p.country.toLowerCase()));
      document.getElementById('travel-stat-places-count').textContent = places.length;
      document.getElementById('travel-stat-countries-badge').textContent = `${countries.size} Countries`;

      // 4. Total Spent
      const totalSpent = expenses.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);
      document.getElementById('travel-stat-spent-count').textContent = `₹${totalSpent.toLocaleString('en-IN')}`;
      
      const totalBudget = trips.reduce((sum, t) => sum + (Number(t.budget) || 0), 0);
      const budgetPct = totalBudget > 0 ? Math.round((totalSpent / totalBudget) * 100) : 0;
      document.getElementById('travel-stat-spent-badge').textContent = `${budgetPct}% of budgeted cost`;

      // 5. Photos Captured
      document.getElementById('travel-stat-photos-count').textContent = photos.length;

      // Sync map footprint summary
      document.getElementById('travel-map-countries-count').textContent = countries.size;
      document.getElementById('travel-map-cities-count').textContent = places.length;
      document.getElementById('travel-map-trips-count').textContent = trips.length;
      document.getElementById('travel-map-days-count').textContent = totalDays;
    ...
    renderCircularRings() {
      ...
      document.getElementById('travel-lifestyle-steps-val').textContent = '7,892';
      document.getElementById('travel-lifestyle-workout-val').textContent = '3 / 5';
      document.getElementById('travel-lifestyle-water-val').textContent = '6 / 8 cups';
      document.getElementById('travel-lifestyle-sleep-val').textContent = '7h 10m';
    ...
    renderPlans() {
      const trips = this.app.state.travelTrips || [];
      const searchQuery = document.getElementById('travel-plans-search').value.toLowerCase().trim();
      ...
      if (filtered.length === 0) {
        ...
        `;
        document.getElementById('travel-plans-detail-drawer').innerHTML = `
    ...
    bindTripDetailDrawerActions(trip) {
      document.getElementById('travel-btn-edit-trip').addEventListener('click', () => {
        document.getElementById('travel-trip-id').value = trip.id;
        document.getElementById('travel-input-dest').value = trip.destination;
        document.getElementById('travel-input-start').value = trip.startDate;
        document.getElementById('travel-input-end').value = trip.endDate;
        document.getElementById('travel-input-budget').value = trip.budget;
        document.getElementById('travel-input-travelers').value = trip.travelers || 1;
        document.getElementById('travel-input-image').value = trip.coverImage || '';
        
        document.getElementById('travel-modal-title').textContent = 'Edit Trip Plan';
      ...
      document.getElementById('travel-btn-save-lodging-details').addEventListener('click', () => {
        const lodging = document.getElementById('travel-detail-lodging').value.trim();
        const transport = document.getElementById('travel-detail-transport').value.trim();
      ...
      document.getElementById('travel-add-itinerary-form').addEventListener('submit', (e) => {
        e.preventDefault();
        const day = Number(document.getElementById('itinerary-input-day').value) || 1;
        const time = document.getElementById('itinerary-input-time').value.trim();
        const activity = document.getElementById('itinerary-input-activity').value.trim();
        const location = document.getElementById('itinerary-input-loc').value.trim();
      ...
      document.getElementById('travel-add-packing-form').addEventListener('submit', (e) => {
        e.preventDefault();
        const name = document.getElementById('packing-input-name').value.trim();
        const cat = document.getElementById('packing-input-cat').value;
      ...
      document.getElementById('travel-btn-save-notes').addEventListener('click', () => {
        const text = document.getElementById('travel-note-editor').value.trim();
      ...
      document.getElementById('travel-add-document-form').addEventListener('submit', (e) => {
        e.preventDefault();
        const name = document.getElementById('doc-input-name').value.trim();
        const type = document.getElementById('doc-input-type').value;
        const url = document.getElementById('doc-input-url').value.trim();
    ...
    renderLifestyleProgress() {
      document.getElementById('lifestyle-input-steps').value = 7892;
      document.getElementById('lifestyle-input-workouts-done').value = 3;
      document.getElementById('lifestyle-input-workouts-target').value = 5;
      document.getElementById('lifestyle-input-water-done').value = 6;
      document.getElementById('lifestyle-input-water-target').value = 8;
      document.getElementById('lifestyle-input-sleep-hrs').value = 7;
      document.getElementById('lifestyle-input-sleep-mins').value = 10;

      this.renderLargeGauges(78, 60, 75, 71);
    },

    renderLargeGauges(stepsPct, workPct, waterPct, sleepPct) {
      document.getElementById('lifestyle-desc-steps').textContent = `7,892 / 10,000 steps`;
      this.drawLargeGauge('lifestyle-gauge-steps', stepsPct, 'var(--green)');

      document.getElementById('lifestyle-desc-workouts').textContent = `3 / 5 workouts`;
      this.drawLargeGauge('lifestyle-gauge-workouts', workPct, 'var(--blue)');

      document.getElementById('lifestyle-desc-water').textContent = `6 / 8 cups water`;
      this.drawLargeGauge('lifestyle-gauge-water', waterPct, 'var(--teal)');

      document.getElementById('lifestyle-desc-sleep').textContent = `7h 10m / 10h sleep`;
    ...
    bindLifestyleEvents() {
      ...
      form.addEventListener('submit', (e) => {
        ...
        const steps = Number(document.getElementById('lifestyle-input-steps').value) || 0;
        const workDone = Number(document.getElementById('lifestyle-input-workouts-done').value) || 0;
        const workTarget = Number(document.getElementById('lifestyle-input-workouts-target').value) || 5;
        const waterDone = Number(document.getElementById('lifestyle-input-water-done').value) || 0;
        const waterTarget = Number(document.getElementById('lifestyle-input-water-target').value) || 8;
        const sleepHrs = Number(document.getElementById('lifestyle-input-sleep-hrs').value) || 0;
        const sleepMins = Number(document.getElementById('lifestyle-input-sleep-mins').value) || 0;
    ...
    bindBucketListEvents() {
      ...
      if (form) {
        form.addEventListener('submit', (e) => {
          e.preventDefault();
          const dest = document.getElementById('bucket-input-dest').value.trim();
          const country = document.getElementById('bucket-input-country').value.trim();
          const url = document.getElementById('bucket-input-image').value.trim();
    ...
    bindExpenseEvents() {
      ...
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        const tripId = document.getElementById('expense-input-trip').value;
        ...
        const desc = document.getElementById('expense-input-desc').value.trim();
    ...
    bindGalleryEvents() {
      ...
      if (form) {
        form.addEventListener('submit', (e) => {
          e.preventDefault();
          const tripId = document.getElementById('gallery-input-trip').value;
          const url = document.getElementById('gallery-input-url').value.trim();
          const caption = document.getElementById('gallery-input-caption').value.trim();
    ...
    renderMapView() {
      ...
      if (places.length === 0) {
        listContainer.innerHTML = `<div class="empty-state" style="font-size:0.75rem; text-align:center; padding:20px 0;">No locations logged yet.</div>`;
    ...
    bindVisitedPlacesEvents() {
      ...
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        const city = document.getElementById('visited-input-city').value.trim();
        const country = document.getElementById('visited-input-country').value.trim();
        const lat = parseFloat(document.getElementById('visited-input-lat').value) || 0;
        const lng = parseFloat(document.getElementById('visited-input-lng').value) || 0;
    ...
    bindTripCrudEvents() {
      ...
      if (addPlansBtn) {
        addPlansBtn.addEventListener('click', () => {
          ...
          document.getElementById('travel-trip-id').value = '';
          document.getElementById('travel-modal-title').textContent = 'Plan New Trip';
          this.toggleModal('travel-trip-modal-overlay', true);
        });
      }
      ...
      if (form) {
        form.addEventListener('submit', (e) => {
          e.preventDefault();
          const id = document.getElementById('travel-trip-id').value;
          const dest = document.getElementById('travel-input-dest').value.trim();
          const start = document.getElementById('travel-input-start').value;
          const end = document.getElementById('travel-input-end').value;
          const budget = parseFloat(document.getElementById('travel-input-budget').value) || 0;
          const travelers = parseInt(document.getElementById('travel-input-travelers').value) || 1;
          const cover = document.getElementById('travel-input-image').value.trim();
