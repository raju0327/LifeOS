...
const FinanceModule = {
  ...
  renderBudgetsList() {
    ...
    const addCatBtn = document.getElementById('finance-add-category-btn');
    if (addCatBtn) {
      if (this.app.state.user.role === 'Super Admin') {
    ...
    const spentMap = {};
    Object.keys(categories).forEach(k => { spentMap[k] = 0; });
    ...
    Object.keys(categories)
      ...
      .forEach(k => {
        ...
        budgetHtml += `
          <div class="budget-card" onclick="window.LifeOS.modules.finance.openCategoryDetails('${k}')">
            <div style="display: flex; justify-content: space-between; align-items: center; font-size: 0.75rem; margin-bottom: 6px;">
              <span style="font-weight: 600; display: flex; align-items: center; gap: 8px;">
                <i class="fas ${categories[k].icon}" style="color: ${categories[k].color};"></i>
                ${categories[k].name}
              </span>
              <span style="font-weight: 700;">
                ${currency}${spent.toLocaleString()} <span style="font-size: 0.65rem; color: var(--text-muted); font-weight: 500;">of ${currency}${limit || '∞'} limit</span>
              </span>
            </div>
            ...
            <span style="font-size: 0.58rem; color: var(--text-muted); display: block; text-align: right;">${pct.toFixed(0)}% consumed</span>
          </div>
        `;
      });

    container.innerHTML = budgetHtml || `<div style="text-align: center; color: var(--text-muted); font-size: 0.72rem; padding: 20px 0;">No budgets configured.</div>`;

    const ratioBadge = document.getElementById('finance-total-budget-ratio');
    const overallBar = document.getElementById('finance-total-budget-progress');
    const spentText = document.getElementById('finance-total-budget-spent');
    const limitText = document.getElementById('finance-total-budget-limit');

    const totalPct = totalLimit > 0 ? (totalSpent / totalLimit) * 100 : 0;
    if (ratioBadge) ratioBadge.textContent = `${totalPct.toFixed(0)}% Used`;
    if (overallBar) {
      overallBar.style.width = `${Math.min(totalPct, 100)}%`;
      overallBar.style.backgroundColor = totalPct >= 95 ? 'var(--red)' : (totalPct >= 80 ? 'var(--orange)' : 'var(--primary)');
    }
    if (spentText) spentText.textContent = `${currency}${totalSpent.toLocaleString(undefined, {maximumFractionDigits:0})} Spent`;
    if (limitText) limitText.textContent = `of ${currency}${totalLimit.toLocaleString(undefined, {maximumFractionDigits:0})} limit`;
  },
